...
function get_students()
{
    ...
    const Sbubble = document.getElementById("Sbubble");
    Sbubble.innerHTML = Sslider.value;
    const new_Position = Srange.clientWidth/Srange.max*Srange.value;
    Sbubble.style.paddingLeft = new_Position + Number(135) +'px';
...
function get_cpu()
{
    ...
    Cbubble.innerHTML = Cslider.value;
    const new_Position = Crange.clientWidth/Crange.max*Crange.value;
    Cbubble.style.paddingLeft = new_Position + Number(140) - (Number(10)/Crange.value) +'px';
    console.log(new_Position);
    return update();
}
